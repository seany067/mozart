...
def test_track_timings(track: Track, sample_audio: AudioClip) -> None:
    track.add_clip(sample_audio, 1, 3)
    assert track.will_clash(Timing(1.1, 1.8)) is not None
    assert track.will_clash(Timing(2, 1)) is not None
    assert track.will_clash(Timing(2, 1.1)) is not None
    assert track.will_clash(Timing(1, 1)) is not None
    assert track.will_clash(Timing(0.9, 1.1)) is not None

    assert track.will_clash(Timing(4, 1)) is None


def test_track_add_clip_throws(track: Track, sample_audio: AudioClip) -> None:
    track.add_clip(sample_audio, 1, 3)
    try:
        track.add_clip(sample_audio, 2, 1)
