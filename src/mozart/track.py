...
class Track:
    ...
    def _will_clash(self, timing: Timing) -> bool:
        keys: List[AudioClip] = sorted(list(self.audio_track.keys()), key=self.audio_track.get)
        key_i = bisect.bisect_left(keys, timing.start_time, key=lambda x: self.audio_track[x].start_time) - 1
